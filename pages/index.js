...
export default function Home(props) {
  //hcgData is being retried from the database
  ...
  /* 
     Every time the page renders this functions runs, retries the current lot
  ...
  */
  ...
  useEffect(() => {
    //Retrieves the data every time the page reloads
    const getLot = async () => {
              ...
              {openInput ? (
                <>
                  <div className="space-y-2" ref={ref}>
                    <div>
                      {/* Input field that renders conditional based on a double click */}
                      <input
                        placeholder="Enter New Lot Here"
                        ref={inputRef}
                        className=" appearance-none focus:outline-none border border-white text-black rounded"
                        onChange={newHcgLotSubmitHandler}
                      ></input>
                    </div>
                    <div className="flex justify-center">
                      <button
                        onClick={updateLot}
                        className=" bg-teal-500 hover:bg-teal-400 text-xs py-1 px-2 mb-1 rounded-2xl"
                      >
                        {" "}
                        Record New Lot
                      </button>
